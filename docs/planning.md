...
- **버그**: `app/socket_listener.py`의 `create_app()`(Slack `auth.test` 호출 포함)이 재시도 루프 바깥에 있어서, 부팅 직후 네트워크 없는 상태에서 예외가 나면 프로세스가 그냥 죽었음(바깥 bash `while true`가 우연히 되살려주긴 했지만 의도한 동작은 아니었음). 초기화까지 루프 안으로 옮기고, 고정 5초 대기를 5→10→20→40→60초 지수 백오프로 바꿈.
- **`deploy/termux-boot/*.sh`에 네트워크 대기 추가** — `/dev/tcp`(bash 내장, curl/ping 불필요)로 최대 60초 폴링 후 앱 시작.
- **로그 무한 증가 수정** — `*-crash.log`는 매 재시작마다 최근 500줄로 자르도록, `deploy.sh`의 `.out` 리다이렉트는 append(`>>`)에서 매 배포마다 새로 쓰기(`>`)로 변경. (`app/logging_config.py`의 `RotatingFileHandler`는 원래부터 회전되고 있었음 — 이건 그 밖의 두 로그 경로 얘기.)
- **확인**: ingest/socket_listener 자체가 죽으면 재시작 안 되는 문제는 애초에 `while true` 루프로 이미 처리되어 있었음 — 재확인만 하고 변경 없음.
- **미해결(코드로 못 함)**: 배터리 최적화 "제한 없음"이 삼성 기기에서 실제로 유지되는지는 문서로 검증 불가 — README에 `*-crash.log`가 재부팅/업데이트와 무관하게 계속 쌓이면 다시 풀렸다는 신호라고 진단 팁만 추가해둠.
